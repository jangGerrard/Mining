...
Timeline._Band = function(B, G, C) {
	...
	this._labeller = ("labeller" in G) ? G.labeller : (("createLabeller" in B
			.getUnit()) ? B.getUnit().createLabeller(this._locale,
			this._timeZone) : new Timeline.GregorianDateLabeller(this._locale,
			this._timeZone));
	this._theme = G.theme;
	...
	SimileAjax.DOM.registerEventWithObject(this._keyboardInput, "keydown",
			this, "_onKeyDown");
	SimileAjax.DOM.registerEventWithObject(this._keyboardInput, "keyup", this,
			"_onKeyUp");
	this._div = this._timeline.getDocument().createElement("div");
	this._div.id = "timeline-band-" + C;
	this._div.className = "timeline-band timeline-band-" + C;
	this._timeline.addDiv(this._div);
	SimileAjax.DOM.registerEventWithObject(this._div, "mousedown", this,
			"_onMouseDown");
	SimileAjax.DOM.registerEventWithObject(this._div, "mousemove", this,
			"_onMouseMove");
	SimileAjax.DOM.registerEventWithObject(this._div, "mouseup", this,
			"_onMouseUp");
	SimileAjax.DOM.registerEventWithObject(this._div, "mouseout", this,
			"_onMouseOut");
	SimileAjax.DOM.registerEventWithObject(this._div, "dblclick", this,
			"_onDblClick");
	var F = this._theme != null ? this._theme.mouseWheel : "scroll";
	if (F === "zoom" || F === "scroll" || this._zoomSteps) {
		if (SimileAjax.Platform.browser.isFirefox) {
			SimileAjax.DOM.registerEventWithObject(this._div, "DOMMouseScroll",
					this, "_onMouseScroll");
		} else {
			SimileAjax.DOM.registerEventWithObject(this._div, "mousewheel",
					this, "_onMouseScroll");
		}
	}
...
Timeline._Band.prototype.checkAutoWidth = function() {
	...
	var A = C ? this._theme.event.overviewTrack.autoWidthMargin
			: this._theme.event.track.autoWidthMargin;
	var B = Math
			.ceil((this._eventTracksNeeded + A) * this._eventTrackIncrement);
	B += C ? this._theme.event.overviewTrack.offset
...
Timeline._Band.prototype.createLayerDiv = function(C, A) {
	...
	D.className = "timeline-band-layer"
			+ (typeof A == "string" ? (" " + A) : "");
	D.style.zIndex = C;
...
Timeline._Band.prototype.scrollToCenter = function(A, C) {
	...
	if (B < -this._viewLength / 2) {
		...
		if (B > 3 * this._viewLength / 2) {
			this.setCenterVisibleDate(this.pixelOffsetToDate(B
					- this._viewLength));
		}
	}
	this._autoScroll(Math.round(this._viewLength / 2
			- this._ether.dateToPixelOffset(A)), C);
};
...
Timeline.CompactEventPainter.prototype.paint = function() {
	...
	var L = O.event;
	var G = {
		trackOffset : "trackOffset" in this._params ? this._params.trackOffset
				: 10,
		trackHeight : "trackHeight" in this._params ? this._params.trackHeight
				: 10,
		tapeHeight : O.event.tape.height,
		tapeBottomMargin : "tapeBottomMargin" in this._params ? this._params.tapeBottomMargin
				: 2,
		labelBottomMargin : "labelBottomMargin" in this._params ? this._params.labelBottomMargin
				: 5,
		labelRightMargin : "labelRightMargin" in this._params ? this._params.labelRightMargin
				: 5,
		defaultIcon : L.instant.icon,
		defaultIconWidth : L.instant.iconWidth,
		defaultIconHeight : L.instant.iconHeight,
		customIconWidth : "iconWidth" in this._params ? this._params.iconWidth
				: L.instant.iconWidth,
		customIconHeight : "iconHeight" in this._params ? this._params.iconHeight
				: L.instant.iconHeight,
		iconLabelGap : "iconLabelGap" in this._params ? this._params.iconLabelGap
				: 2,
		iconBottomMargin : "iconBottomMargin" in this._params ? this._params.iconBottomMargin
				: 2
	};
	if ("compositeIcon" in this._params) {
		G.compositeIcon = this._params.compositeIcon;
		G.compositeIconWidth = this._params.compositeIconWidth
				|| G.customIconWidth;
		G.compositeIconHeight = this._params.compositeIconHeight
				|| G.customIconHeight;
	} else {
	...
	G.defaultStackIcon = "icon" in this._params.stackConcurrentPreciseInstantEvents ? this._params.stackConcurrentPreciseInstantEvents.icon
			: G.defaultIcon;
	G.defaultStackIconWidth = "iconWidth" in this._params.stackConcurrentPreciseInstantEvents ? this._params.stackConcurrentPreciseInstantEvents.iconWidth
			: G.defaultIconWidth;
	G.defaultStackIconHeight = "iconHeight" in this._params.stackConcurrentPreciseInstantEvents ? this._params.stackConcurrentPreciseInstantEvents.iconHeight
			: G.defaultIconHeight;
	...
	var Q = (this._highlightMatcher != null) ? this._highlightMatcher
			: function(S) {
				return -1;
			};
	var F = N.getEventIterator(B, D);
	var H = "stackConcurrentPreciseInstantEvents" in this._params
			&& typeof this._params.stackConcurrentPreciseInstantEvents == "object";
	var P = "collapseConcurrentPreciseInstantEvents" in this._params
			&& this._params.collapseConcurrentPreciseInstantEvents;
	if (P || H) {
		var M = [];
		var A = null;
		while (F.hasNext()) {
			...
			if (R(E)) {
				if (!E.isInstant() || E.isImprecise()) {
					...
					if (A != null
							&& A.getStart().getTime() == E.getStart().getTime()) {
						M[M.length - 1].push(E);
					} else {
						M.push([ E ]);
						A = E;
		...
		for (var J = 0; J < M.length; J++) {
			...
			if (K.length == 1) {
				...
				if (H) {
					this.paintStackedPreciseInstantEvents(K, G,
							this._params.theme, C);
				} else {
					this.paintCompositePreciseInstantEvents(K, G,
							this._params.theme, C);
				}
			}
		}
...
Timeline.CompactEventPainter.prototype.paintPreciseInstantEvent = function(H,
		F, B, A) {
	var C = {
...
Timeline.CompactEventPainter.prototype.paintCompositePreciseInstantEvents = function(
		J, H, D, B) {
	var K = J[0];
	var A = [];
	...
	var M = {
		text : String.substitute(this._params.compositeEventLabelTemplate,
				[ J.length ])
	};
...
Timeline.CompactEventPainter.prototype.paintStackedPreciseInstantEvents = function(
		T, j, c, E) {
	var S = "limit" in this._params.stackConcurrentPreciseInstantEvents ? this._params.stackConcurrentPreciseInstantEvents.limit
			: 10;
	var G = "moreMessageTemplate" in this._params.stackConcurrentPreciseInstantEvents ? this._params.stackConcurrentPreciseInstantEvents.moreMessageTemplate
			: "%0 More Events";
	var Q = S <= T.length - 2;
	var B = this._band;
	var L = function(i) {
	...
	var d = function(u) {
		...
		var v = I
				._paintEventLabel({
					...
					text : w.text
				}, F + w.labelLeft, Z + w.top, w.labelSize.width,
						w.labelSize.height, c);
		...
		s.className = "timeline-event-icon"
				+ ("className" in w.iconData ? (" " + w.iconData.className)
						: "");
...
Timeline.CompactEventPainter.prototype.paintImpreciseInstantEvent = function(I,
		G, B, A) {
	var C = {
...
Timeline.CompactEventPainter.prototype.paintPreciseDurationEvent = function(I,
		G, B, A) {
	var C = {
		tooltip : I.getProperty("tooltip") || I.getText()
	};
...
Timeline.CompactEventPainter.prototype.paintImpreciseDurationEvent = function(
		I, G, B, A) {
	var C = {
		tooltip : I.getProperty("tooltip") || I.getText()
	};
...
Timeline.CompactEventPainter.prototype.paintTapeIconLabel = function(V, O, S,
		I, a, X, c, Z) {
	var R = this._band;
	var F = function(e) {
	...
	var D = {};
	D.labelElmtData = this._paintEventLabel(O, a, d + U, K + b, H.width,
			H.height, c);
	if (S != null) {
		if ("latestStart" in S || "earliestEnd" in S) {
			D.impreciseTapeElmtData = this._paintEventTape(O, S, X.tapeHeight,
					K, F(S.start), F(S.end), c.event.duration.impreciseColor,
					c.event.duration.impreciseOpacity, X, c);
		}
		if (!S.isInstant && "start" in S && "end" in S) {
			D.tapeElmtData = this._paintEventTape(O, S, X.tapeHeight, K, d,
					F("earliestEnd" in S ? S.earliestEnd : S.end), S.color,
					100, X, c);
		}
	}
	if (I != null) {
...
Timeline.CompactEventPainter.prototype._paintEventIcon = function(C, D, H, G,
		E, F) {
...
Timeline.CompactEventPainter.prototype._paintEventLabel = function(E, I, C, F,
		A, G, D) {
...
Timeline.CompactEventPainter.prototype._paintEventTape = function(G, H, K, J,
		D, A, E, C, I, F) {
	var B = A - D;
	...
	if ("backgroundImage" in H && typeof H.backgroundImage == "string") {
		...
		L.style.backgroundRepeat = ("backgroundRepeat" in H && typeof H.backgroundRepeat == "string") ? H.backgroundRepeat
				: "repeat";
	}
...
Timeline.CompactEventPainter.prototype._onClickMultiplePreciseInstantEvent = function(
		E, A, B) {
	var F = SimileAjax.DOM.getPageCoordinates(E);
	this._showBubble(F.left + Math.ceil(E.offsetWidth / 2), F.top
			+ Math.ceil(E.offsetHeight / 2), B);
	var D = [];
...
Timeline.CompactEventPainter.prototype._onClickInstantEvent = function(C, A, B) {
	...
	this._showBubble(D.left + Math.ceil(C.offsetWidth / 2), D.top
			+ Math.ceil(C.offsetHeight / 2), [ B ]);
...
Timeline.CompactEventPainter.prototype.showBubble = function(A) {
	...
	if (B) {
		...
		this._showBubble(C.left + B.offsetWidth / 2,
				C.top + B.offsetHeight / 2, [ A ]);
	}
};
Timeline.CompactEventPainter.prototype._showBubble = function(A, F, B) {
	...
	SimileAjax.WindowManager.cancelPopups();
	SimileAjax.Graphics.createBubbleForContentAndPoint(E, A, F,
...
Timeline.SpanHighlightDecorator.prototype.paint = function() {
	...
	if (this._unit.compare(this._startDate, A) < 0
...
Timeline.DetailedEventPainter.prototype.paint = function() {
	...
	var A = (this._highlightMatcher != null) ? this._highlightMatcher
			: function(K) {
				return -1;
			};
	...
	this._lineLayer.style.display = "block";
	this._eventLayer.style.display = "block";
	this._band.updateEventTrackInfo(this._lowerTracks.length
			+ this._upperTracks.length, F.trackIncrement);
};
...
Timeline.DetailedEventPainter.prototype.paintDurationEvent = function(B, C, D,
		A) {
	if (B.isImprecise()) {
		this.paintImpreciseDurationEvent(B, C, D, A);
	} else {
...
Timeline.DetailedEventPainter.prototype.paintPreciseInstantEvent = function(L,
		P, S, Q) {
	...
	var N = Math.round(P.trackOffset + O * P.trackIncrement + P.trackHeight / 2
			- E.height / 2);
...
Timeline.DetailedEventPainter.prototype.paintImpreciseInstantEvent = function(
		O, S, W, T) {
	...
	var E = this._paintEventTape(O, H, K, B, W.event.instant.impreciseColor,
			W.event.instant.impreciseOpacity, S, W);
	...
	var Q = Math.round(S.trackOffset + R * S.trackIncrement + S.trackHeight / 2
			- G.height / 2);
...
Timeline.DetailedEventPainter.prototype.paintPreciseDurationEvent = function(K,
		O, T, Q) {
	...
	var M = Math.round(O.trackOffset + N * O.trackIncrement + O.trackHeight / 2
			- C.height / 2);
...
Timeline.DetailedEventPainter.prototype.paintImpreciseDurationEvent = function(
		M, T, Y, V) {
	...
	var R = this._paintEventTape(M, D, H, A, Y.event.duration.impreciseColor,
			Y.event.duration.impreciseOpacity, T, Y);
	...
	var P = Math.round(T.trackOffset + Q * T.trackIncrement + T.trackHeight / 2
			- C.height / 2);
...
Timeline.DetailedEventPainter.prototype._findFreeTrackForText = function(C, A,
		I) {
	var B;
	var E;
	var F;
...
Timeline.DetailedEventPainter.prototype._findFreeLowerTrackForText = function(
		A, C) {
...
Timeline.DetailedEventPainter.prototype._findFreeUpperTrackForText = function(
		A, C) {
...
Timeline.DetailedEventPainter.prototype._paintEventLine = function(J, E, D, A,
		G, F) {
	var H = Math
			.round(G.trackOffset + D * G.trackIncrement + G.trackHeight / 2);
...
Timeline.DetailedEventPainter.prototype._paintEventIcon = function(J, B, C, F,
		E) {
	var H = J.getIcon();
	H = H != null ? H : F.icon;
...
Timeline.DetailedEventPainter.prototype._paintEventLabel = function(I, J, C, F,
		A, G, E) {
	var H = this._timeline.getDocument();
...
Timeline.DetailedEventPainter.prototype._paintEventTape = function(L, B, D, A,
		G, C, I, H) {
	var F = A - D;
...
Timeline.DetailedEventPainter.prototype._onClickDurationEvent = function(F, B,
		C) {
	if ("pageX" in B) {
		var A = B.pageX;
		var E = B.pageY;
...
Timeline.DetailedEventPainter.prototype._showBubble = function(B, E, C) {
	...
	SimileAjax.WindowManager.cancelPopups();
	SimileAjax.Graphics.createBubbleForContentAndPoint(D, B, E, A.width, null,
			A.maxHeight);
};
...
Timeline.GregorianEtherPainter.prototype.paint = function() {
	...
	SimileAjax.DateTime.roundDownToInterval(C, this._unit, A, this._multiple,
...
Timeline.HotZoneGregorianEtherPainter.prototype.paint = function() {
	...
	for (var H = D; H <= E; H++) {
		...
		SimileAjax.DateTime.roundDownToInterval(K, G.unit, I, G.multiple,
				this._theme.firstDayOfWeek);
		SimileAjax.DateTime.roundUpToInterval(F, G.unit, I, G.multiple,
				this._theme.firstDayOfWeek);
		while (K.getTime() < F.getTime()) {
			this._intervalMarkerLayout.createIntervalMarker(K, L, G.unit,
					this._markerLayer, this._lineLayer);
			J(K, G);
		}
	}
...
Timeline.YearCountEtherPainter.prototype.paint = function() {
	...
	var A = function(G) {
		for (var H = 0; H < C._multiple; H++) {
			SimileAjax.DateTime
					.incrementByInterval(G, SimileAjax.DateTime.YEAR);
		}
	};
	var D = {
	...
	while (B.getTime() < F.getTime()) {
		this._intervalMarkerLayout.createIntervalMarker(B, D,
				SimileAjax.DateTime.YEAR, this._markerLayer, this._lineLayer);
		A(B);
	}
	this._markerLayer.style.display = "block";
	this._lineLayer.style.display = "block";
...
Timeline.GregorianDateLabeller.prototype.labelPrecise = function(A) {
	return SimileAjax.DateTime.removeTimeZoneOffset(A, this._timeZone)
			.toUTCString();
};
Timeline.GregorianDateLabeller.prototype.defaultLabelInterval = function(B, C) {
	...
	case SimileAjax.DateTime.DAY:
		D = Timeline.GregorianDateLabeller.getMonthName(B.getUTCMonth(),
				this._locale)
				+ " " + B.getUTCDate();
		break;
	...
	case SimileAjax.DateTime.MILLENNIUM:
		...
		F = (C == SimileAjax.DateTime.MONTH)
				|| (C == SimileAjax.DateTime.DECADE && E % 100 == 0)
				|| (C == SimileAjax.DateTime.CENTURY && E % 1000 == 0);
		break;
	default:
		D = B.toUTCString();
	}
...
Timeline.OriginalEventPainter.prototype.paintDurationEvent = function(B, C, D,
		A) {
	if (B.isImprecise()) {
		this.paintImpreciseDurationEvent(B, C, D, A);
	} else {
...
Timeline.OriginalEventPainter.prototype.paintPreciseInstantEvent = function(N,
		S, V, T) {
	...
	var R = Math.round(S.trackOffset + P * S.trackIncrement + S.trackHeight / 2
			- D.height / 2);
...
Timeline.OriginalEventPainter.prototype.paintImpreciseInstantEvent = function(
		P, U, Z, W) {
	...
	var E = this._paintEventTape(P, R, K, B, V,
			Z.event.instant.impreciseOpacity, U, Z, 0);
...
Timeline.OriginalEventPainter.prototype.paintPreciseDurationEvent = function(M,
		R, W, T) {
	var X = this._timeline.getDocument();
	var K = M.getText();
	var G = M.getStart();
	...
	var Q = Math.round(R.trackOffset + O * R.trackIncrement
			+ W.event.tape.height);
	var S = M.getColor();
...
Timeline.OriginalEventPainter.prototype.paintImpreciseDurationEvent = function(
		O, W, b, Y) {
	...
	var T = Math.round(W.trackOffset + R * W.trackIncrement
			+ b.event.tape.height);
	var X = O.getColor();
	X = X != null ? X : b.event.duration.color;
	var U = this._paintEventTape(O, R, K, A, b.event.duration.impreciseColor,
			b.event.duration.impreciseOpacity, W, b, 0);
...
Timeline.OriginalEventPainter.prototype._encodeEventElID = function(B, A) {
	return Timeline.EventUtils
			.encodeEventElID(this._timeline, this._band, B, A);
};
...
Timeline.OriginalEventPainter.prototype._paintEventIcon = function(K, B, C, G,
		F, D) {
	var I = K.getIcon();
	I = I != null ? I : G.icon;
	var J;
...
Timeline.OriginalEventPainter.prototype._paintEventLabel = function(K, L, D, H,
		A, J, G, E, C) {
...
Timeline.OriginalEventPainter.prototype._paintEventTape = function(N, B, D, A,
		G, C, J, I, M) {
	var F = A - D;
...
Timeline.OriginalEventPainter.prototype._createHighlightDiv = function(A, D, F,
		B) {
	var G = null;
	if (A >= 0) {
...
Timeline.OriginalEventPainter.prototype._onClickDurationEvent = function(F, B,
		C) {
	if ("pageX" in B) {
		var A = B.pageX;
		var E = B.pageY;
...
Timeline.OriginalEventPainter.prototype._fireEventPaintListeners = function(D,
		A, C) {
...
Timeline.OverviewEventPainter.prototype.paintDurationEvent = function(L, K, H,
		B) {
...
Timeline.OverviewEventPainter.prototype._paintEventTape = function(K, B, D, L,
		E, C, H, G, F) {
...
Timeline.OverviewEventPainter.prototype._paintEventTick = function(J, C, D, B,
		G, F) {
...
Timeline.DefaultEventSource.prototype.loadXML = function(G, A) {
	...
	while (D != null) {
		if (D.nodeType == 1) {
			...
			var B = (D.getAttribute("isDuration") === null && D
					.getAttribute("durationEvent") === null)
					|| D.getAttribute("isDuration") == "false"
					|| D.getAttribute("durationEvent") == "false";
...
Timeline.DefaultEventSource.prototype.loadJSON = function(H, B) {
	...
	if (H && H.events) {
		...
		for (var G = 0; G < H.events.length; G++) {
			var A = H.events[G];
			var C = A.isDuration
					|| (A.durationEvent != null && !A.durationEvent);
...
Timeline.DefaultEventSource.prototype.loadSPARQL = function(I, B) {
	...
	while (F != null) {
		if (F.nodeType == 1) {
			...
			var A = F.firstChild;
			while (A != null) {
				if (A.nodeType == 1 && A.firstChild != null
						&& A.firstChild.nodeType == 1
						&& A.firstChild.firstChild != null
						&& A.firstChild.firstChild.nodeType == 3) {
			...
			var C = (D["isDuration"] === null && D["durationEvent"] === null)
					|| D["isDuration"] == "false"
...
Timeline.DefaultEventSource.Event = function(A) {
	...
	this._latestStart = (A.latestStart != null) ? A.latestStart
			: (A.instant ? this._end : this._start);
...
Timeline.display_version = Timeline.version + " (with Ajax lib "
		+ Timeline.ajax_lib_version + ")";
Timeline.strings = {};
...
Timeline.createBandInfo = function(F) {
	...
	var B = ("overview" in F && F.overview) ? "overview"
			: ("layout" in F ? F.layout : "original");
	var E;
...
Timeline.getDefaultTheme = function() {
	if (Timeline._defaultTheme == null) {
		Timeline._defaultTheme = Timeline.ClassicTheme.create(Timeline
				.getDefaultLocale());
	}
...
Timeline._Impl = function(C, B, D, E, A) {
	...
	this.autoWidthAnimationTime = B && B[0] && B[0].theme
			&& B[0].theme.autoWidthAnimationTime;
	this.timelineID = A;
...
Timeline._Impl.prototype._autoWidthCheck = function(C) {
	...
	function E() {
		...
		if (B) {
			...
			var H = {};
			H[G] = D + "px";
			SimileAjax.jQuery(A._containerDiv).animate(H,
					A.autoWidthAnimationTime, "linear", function() {
...
Timeline._Impl.prototype._initialize = function() {
	...
	var A = SimileAjax.Graphics.createTranslucentImage(Timeline.urlPrefix
			+ (this.isHorizontal() ? "images/copyright-vertical.png"
	...
	for (var B = 0; B < this._bandInfos.length; B++) {
		...
		if ("syncWith" in F) {
			this._bands[B].setSyncWithBand(this._bands[F.syncWith],
					("highlight" in F) ? F.highlight : false);
		}
	}
	if (this.autoWidth) {
	...
	D.contentDiv.innerHTML = "<img src='" + Timeline.urlPrefix
...
Timeline._Impl.prototype.shiftOK = function(C, B) {
	var F = B > 0, A = B < 0;
	if ((F && this.timeline_start == null) || (A && this.timeline_stop == null)
			|| (B == 0)) {
		return (true);
	}
	var H = false;
	...
	for (var E = 0; E < this._bands.length && !D; E++) {
		var G = this._bands[E];
		if (F) {
			D = (E == C ? G.getMinVisibleDateAfterDelta(B) : G
					.getMinVisibleDate()) >= this.timeline_start;
		} else {
			D = (E == C ? G.getMaxVisibleDateAfterDelta(B) : G
					.getMaxVisibleDate()) <= this.timeline_stop;
		}
	}
	if (F) {
