							...
							if (m[1] == ".") {
								...
								if (m[1] == "[") {
									...
									for (var i = 0, rl = r.length; i < rl; i++) {
										var a = r[i], z = a[jQuery.props[m[2]]
												|| m[2]];
										if (z == null
												|| /href|src|selected/
														.test(m[2])) {
									...
									if (m[1] == ":" && m[2] == "nth-child") {
										var merge = {}, tmp = [], test = /(-?)(\d*)n((?:\+|-)?\d*)/
												.exec(m[3] == "even" && "2n"
														...
														&& "2n+1"
														|| !/\D/.test(m[3])
														&& "0n+" + m[3] || m[3]), first = (test[1] + (test[2] || 1)) - 0, last = test[3] - 0;
										for (var i = 0, rl = r.length; i < rl; i++) {
										...
										if (typeof fn == "string") {
											fn = eval("false||function(a,i){return "
													+ fn + ";}");
										}
			...
			.each(
					("blur,focus,load,resize,scroll,unload,click,dblclick,mousedown,mouseup,mousemove,mouseover,mouseout,change,select,submit,keydown,keypress,keyup,error")
						...
						jQuery.fn[name] = function(fn) {
							return fn ? this.bind(name, fn) : this
									.trigger(name);
						};
					});
	...
	for (var D = 0; D < A.length; D++) {
		...
		if (C in E) {
			try {
				...
				SimileAjax.Debug
						.exception("Error firing event of name " + C, F);
			}
		} else {
			if (this._wildcardHandlerName != null
					&& this._wildcardHandlerName in E) {
				try {
					E[this._wildcardHandlerName].apply(E, [ C ]);
				} catch (F) {
					SimileAjax.Debug.exception("Error firing event of name "
							+ C + " to wildcard handler", F);
				}
			}
		}
	}
...
SimileAjax.EventIndex.prototype.getReverseIterator = function(A, B) {
	...
	return new SimileAjax.EventIndex._ReverseIterator(this._events, A, B,
			this._unit);
};
...
SimileAjax.DateTime._dateRegexp = new RegExp("^(-?)([0-9]{4})("
		+ [ "(-?([0-9]{2})(-?([0-9]{2}))?)", "(-?([0-9]{3}))",
				"(-?W([0-9]{2})(-?([1-7]))?)" ].join("|") + ")?$");
...
SimileAjax.DateTime.roundDownToInterval = function(E, B, I, K, A) {
	...
	case SimileAjax.DateTime.WEEK:
		...
		J
				.setTime(J.getTime()
						- G
						* SimileAjax.DateTime.gregorianUnitLengths[SimileAjax.DateTime.DAY]);
		break;
...
SimileAjax.DateTime.roundUpToInterval = function(C, F, D, A, B) {
	...
	if (C.getTime() < E) {
		C
				.setTime(C.getTime()
						+ SimileAjax.DateTime.gregorianUnitLengths[F] * A);
	}
};
			...
			A = function(F, E) {
				if (!SimileAjax.Debug.silent) {
					alert("Caught exception: " + E + "\n\nDetails: "
							+ ("description" in F ? F.description : F));
				}
				throw (F);
			};
		}
	...
	if (F == null) {
		...
		if (typeof F != "object") {
			...
			if ("tag" in F) {
				...
				if (A == null) {
					A = J == "input" ? SimileAjax.DOM
							.createInputElement(F.type) : document
							.createElement(J);
					if (D != null) {
						D.appendChild(A);
					}
								...
								if (B == "type" && A.tagName == "input") {
									...
									if (B == "style") {
										for (n in H) {
											var I = H[n];
											if (n == "float") {
												n = SimileAjax.Platform.browser.isIE ? "styleFloat"
														: "cssFloat";
											}
											A.style[n] = I;
										}
...
SimileAjax.Graphics.pngIsTranslucent = (!SimileAjax.Platform.browser.isIE)
		|| (SimileAjax.Platform.browser.majorVersion > 6);
if (!SimileAjax.Graphics.pngIsTranslucent) {
	SimileAjax.includeCssFile(document, SimileAjax.urlPrefix
			+ "styles/graphics-ie6.css");
}
...
SimileAjax.Graphics.createTranslucentImage = SimileAjax.Graphics.pngIsTranslucent ? SimileAjax.Graphics._createTranslucentImage1
		: SimileAjax.Graphics._createTranslucentImage2;
SimileAjax.Graphics._createTranslucentImageHTML1 = function(A, B) {
	return '<img src="' + A + '"'
			+ (B != null ? ' style="vertical-align: ' + B + ';"' : "") + " />";
};
SimileAjax.Graphics._createTranslucentImageHTML2 = function(A, C) {
	var B = "width: 1px; height: 1px; filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(src='"
			+ A
			+ "', sizingMethod='image');"
			+ (C != null ? " vertical-align: " + C + ";" : "");
	return "<img src='" + A + "' style=\"" + B + '" />';
};
SimileAjax.Graphics.createTranslucentImageHTML = SimileAjax.Graphics.pngIsTranslucent ? SimileAjax.Graphics._createTranslucentImageHTML1
...
SimileAjax.Graphics.createBubbleForPoint = function(B, A, J, N, F) {
	...
	var M = SimileAjax.Graphics.pngIsTranslucent ? "pngTranslucent"
			: "pngNotTranslucent";
	...
	(function() {
		...
		if (B - V - E.borderGraphicSize - E.extraPadding > 0
				&& B + V + E.borderGraphicSize + E.extraPadding < T) {
			var X = B - Math.round(J / 2);
			X = B < (T / 2) ? Math.max(X, E.extraPadding + E.borderGraphicSize)
					: Math.min(X, T - E.extraPadding - E.borderGraphicSize - J);
			if ((F && F == "top")
					|| (!F && (A - E.arrowGraphicTargetOffset - N
							- E.borderGraphicSize - E.extraPadding > 0))) {
				var S = Z("down");
				...
				if ((F && F == "bottom")
						|| (!F && (A + E.arrowGraphicTargetOffset + N
								+ E.borderGraphicSize + E.extraPadding < U))) {
					var S = Z("up");
					S.style.left = (B - V - X) + "px";
					H.style.left = X + "px";
		...
		W = A < (U / 2) ? Math.max(W, E.extraPadding + E.borderGraphicSize)
				: Math.min(W, U - E.extraPadding - E.borderGraphicSize - N);
		if ((F && F == "left")
				|| (!F && (B - E.arrowGraphicTargetOffset - J
						- E.borderGraphicSize - E.extraPadding > 0))) {
			var S = Z("right");
...
SimileAjax.Graphics.createMessageBubble = function(H) {
	...
	if (SimileAjax.Graphics.pngIsTranslucent) {
		...
		E.style.height = "55px";
		E.style.background = "url(" + SimileAjax.urlPrefix
				+ "images/message-bottom-left.png) bottom left no-repeat";
		E.style.paddingLeft = "44px";
		G.appendChild(E);
		var A = H.createElement("div");
		A.style.height = "55px";
		A.style.background = "url(" + SimileAjax.urlPrefix
...
SimileAjax.Graphics.createStructuredDataCopyButton = function(F, B, D, E) {
	...
	if (SimileAjax.Graphics.pngIsTranslucent) {
		...
		G.style.filter = "progid:DXImageTransform.Microsoft.AlphaImageLoader(src='"
				+ F + "', sizingMethod='image')";
	}
	var A;
	if (SimileAjax.Platform.browser.isIE) {
	...
	G.innerHTML = "<textarea rows='1' autocomplete='off' value='none' style='"
			+ A + "' />";
	var C = G.firstChild;
	C.style.width = B + "px";
	C.style.height = D + "px";
...
SimileAjax.History.initialize = function() {
	...
	if (SimileAjax.History.enabled) {
		...
		SimileAjax.DOM.registerEvent(A, "load",
	...
	window
			.setTimeout(
					function() {
						try {
							A.perform();
							SimileAjax.History._listeners.fire(
									"onAfterPerform", [ A ]);
							if (SimileAjax.History.enabled) {
								SimileAjax.History._actions = SimileAjax.History._actions
										.slice(
												0,
								...
								SimileAjax.History._currentIndex++;
								var C = SimileAjax.History._actions.length
								...
								try {
									...
									var D = SimileAjax.History
											.formatHistoryEntryTitle(A.label);
									document.title = D;
								}
							}
						} catch (B) {
							SimileAjax.Debug.exception(B,
									"Error adding action {" + A.label
											+ "} to history");
						}
					}, 0);
};
	...
	try {
		...
		if (F < SimileAjax.History._currentIndex) {
			...
			window
					.setTimeout(
							function() {
								while (SimileAjax.History._currentIndex > F
										&& SimileAjax.History._currentIndex > SimileAjax.History._baseIndex) {
									SimileAjax.History._currentIndex--;
									var G = SimileAjax.History._actions[SimileAjax.History._currentIndex
											- SimileAjax.History._baseIndex];
									try {
										G.undo();
									} catch (H) {
										SimileAjax.Debug.exception(H,
												"History: Failed to undo action {"
														+ G.label + "}");
									}
								}
								SimileAjax.History._listeners.fire(
			...
			if (F > SimileAjax.History._currentIndex) {
				...
				window
						.setTimeout(
								function() {
									while (SimileAjax.History._currentIndex < F
											&& SimileAjax.History._currentIndex
													- SimileAjax.History._baseIndex < SimileAjax.History._actions.length) {
										var G = SimileAjax.History._actions[SimileAjax.History._currentIndex
												- SimileAjax.History._baseIndex];
										try {
											G.perform();
				...
				var A = SimileAjax.History._currentIndex
						- SimileAjax.History._baseIndex - 1;
				var E = (A >= 0 && A < SimileAjax.History._actions.length) ? SimileAjax.History
						.formatHistoryEntryTitle(SimileAjax.History._actions[A].label)
...
SimileAjax.NativeDateUnit.getParser = function(A) {
	...
	return (A == "iso8601" || A == "iso 8601") ? SimileAjax.DateTime.parseIso8601DateTime
...
SimileAjax.WindowManager.initialize = function() {
	...
	SimileAjax.DOM.registerEvent(document.body, "mousedown",
			SimileAjax.WindowManager._onBodyMouseDown);
	SimileAjax.DOM.registerEvent(document.body, "mousemove",
	...
	SimileAjax.DOM.registerEvent(document, "keydown",
			SimileAjax.WindowManager._onBodyKeyDown);
	SimileAjax.DOM.registerEvent(document, "keyup",
...
SimileAjax.WindowManager.cancelPopups = function(A) {
	...
	while (E > 0 && SimileAjax.WindowManager._layers[E].ephemeral) {
		...
		if (D.elmt != null) {
			...
			if (F.x >= B.left && F.x < (B.left + C.offsetWidth) && F.y >= B.top
					&& F.y < (B.top + C.offsetHeight)) {
				break;
			}
		}
		E--;
	...
	if (SimileAjax.WindowManager._dragging) {
		if (A.keyCode == 27) {
			...
			if ((A.keyCode == 17 || A.keyCode == 16)
				...
				SimileAjax.WindowManager._draggingMode = "copy";
				var B = SimileAjax.Graphics
						.createTranslucentImage(SimileAjax.urlPrefix
								+ "images/copy.png");
				B.style.position = "absolute";
				B.style.left = (SimileAjax.WindowManager._ghostCoords.left - 16)
						+ "px";
				B.style.top = (SimileAjax.WindowManager._ghostCoords.top)
						+ "px";
	...
	if (SimileAjax.WindowManager._dragging) {
		if (A.keyCode == 17 || A.keyCode == 16) {
			...
			if (SimileAjax.WindowManager._draggingModeIndicatorElmt != null) {
				document.body
						.removeChild(SimileAjax.WindowManager._draggingModeIndicatorElmt);
			...
			if (Math.abs(J) > 5 || Math.abs(I) > 5) {
				try {
					...
					if ("ghost" in L && L.ghost) {
						...
						SimileAjax.WindowManager._ghostCoords = SimileAjax.DOM
								.getPageCoordinates(P);
						SimileAjax.WindowManager._ghostCoords.left += J;
						SimileAjax.WindowManager._ghostCoords.top += I;
						var K = P.cloneNode(true);
						K.style.position = "absolute";
						K.style.left = SimileAjax.WindowManager._ghostCoords.left
								+ "px";
						K.style.top = SimileAjax.WindowManager._ghostCoords.top
								+ "px";
						K.style.zIndex = 1000;
					...
					SimileAjax.Debug.exception(
							"WindowManager: Error handling mouse down", H);
					SimileAjax.WindowManager._cancelDragging();
				}
			}
		} else {
			try {
				...
				if ("_ghostElmt" in L) {
					...
					if (SimileAjax.WindowManager._draggingModeIndicatorElmt != null) {
						...
						O.style.left = (SimileAjax.WindowManager._ghostCoords.left - 16)
								+ "px";
						O.style.top = SimileAjax.WindowManager._ghostCoords.top
								+ "px";
					}
					if ("droppable" in L && L.droppable) {
						var N = SimileAjax.DOM.getEventPageCoordinates(M);
						var B = SimileAjax.DOM
								.hittest(
										N.x,
										N.y,
										[
						...
						if (B != SimileAjax.WindowManager._potentialDropTarget) {
							if (SimileAjax.WindowManager._dropTargetHighlightElement != null) {
								document.body
										.removeChild(SimileAjax.WindowManager._dropTargetHighlightElement);
								SimileAjax.WindowManager._dropTargetHighlightElement = null;
								SimileAjax.WindowManager._potentialDropTarget = null;
							}
							var A = false;
							if (B != null) {
								if ((!("canDropOn" in L) || L.canDropOn(B))
										&& (!("canDrop" in B) || B
												.canDrop(SimileAjax.WindowManager._draggedElement))) {
									A = true;
								}
							}
							if (A) {
								...
								F.style.height = (B.offsetHeight - E * 2)
										+ "px";
				...
				SimileAjax.Debug.exception(
						"WindowManager: Error handling mouse move", H);
				SimileAjax.WindowManager._cancelDragging();
			}
		}
		...
		try {
			if (SimileAjax.WindowManager._dragging) {
				...
				if ("droppable" in C && C.droppable) {
					...
					if (E != null) {
						if ((!("canDropOn" in C) || C.canDropOn(E))
								&& (!("canDrop" in E) || E
										.canDrop(SimileAjax.WindowManager._draggedElement))) {
							if ("onDropOn" in C) {
								C.onDropOn(E);
							}
							E.ondrop(SimileAjax.WindowManager._draggedElement,
									SimileAjax.WindowManager._draggingMode);
							D = true;
						}
					}
...
SimileAjax.XmlHttp._onReadyStateChange = function(A, D, B) {
	...
	case 4:
		try {
			...
			SimileAjax.Debug.exception(
					"XmlHttp: Error handling onReadyStateChange", C);
		}
		break;
	}
};
